...
(:predicates (at_kitchen_bread ?b - bread-portion)
         ...
         (sandwich_contents ?s - sandwich ?d0 ?d1 ?d2 ?d3 - content-description)
...
(:action make_sandwich
	 :parameters (?s - sandwich ?b - bread-portion ?c0 ?c1 ?c2 ?c3 - content-portion ?d0 ?d1 ?d2 ?d3 - content-description)
	 :precondition (and (at_kitchen_bread ?b)
			    (at_kitchen_content ?c0)
			    (at_kitchen_content ?c1)
			    (at_kitchen_content ?c2)
			    (at_kitchen_content ?c3)
                (not (= ?d0 ?d1))
                (not (= ?d0 ?d2))
                (not (= ?d0 ?d3))
                (not (= ?d1 ?d2))
	 ...
	 :effect (and
		   ...
		   (not (at_kitchen_content ?c3))

		   (at_kitchen_sandwich ?s)
           (sandwich_contents ?s ?d0 ?d1 ?d2 ?d3)
		   (not (notexist ?s))
		   ))
...
(:action serve_sandwich
 	:parameters (?s - sandwich ?d0 ?d1 ?d2 ?d3 - content-description ?c - child ?t - tray ?p - place)
	:precondition (and
               (sandwich_contents ?s ?d0 ?d1 ?d2 ?d3)
		       (ontray ?s ?t)
		       (waiting ?c ?p)
